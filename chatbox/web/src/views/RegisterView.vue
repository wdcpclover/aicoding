  ...
  <div class="auth-wrap">
    <div class="auth-card">
      <header>
        <Logo :size="44" />
        <h1>创建账号</h1>
        <p class="lead">数据存在本地 SQLite，仅本机可见</p>
      </header>

      <form @submit.prevent="onSubmit" autocomplete="on">
        <label>
          <span>用户名</span>
          <input v-model="form.username" autofocus placeholder="2 位以上">
        </label>
        <label>
          <span>密码</span>
          <input v-model="form.password" type="password" placeholder="4 位以上">
        </label>
        <label>
          <span>确认密码</span>
          <input v-model="form.confirm" type="password" placeholder="再输一遍">
        </label>
...
.switch { margin: 18px 0 0; text-align: center; color: var(--text-muted); font-size: 13px; }
