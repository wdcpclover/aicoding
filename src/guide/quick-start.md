...
## Vue 项目目录结构 {#vue-project-directory-structure}
...
├── node_modules/     # npm 依赖包（自动生成，git 忽略）
├── public/           # 静态资源目录
├── src/              # 源代码目录
│   ├── assets/       # 静态资源（图片、样式文件）
...
│   ├── views/        # 页面级组件（路由目标）
│   ├── App.vue       # 根组件
│   └── main.js       # 入口文件，初始化 Vue 并挂载到 DOM
├── .gitignore        # git 忽略配置
├── index.html        # 主 HTML 文件，Vue 挂载点
├── jsconfig.json     # JS 项目配置，辅助编辑器工具
├── package.json      # 依赖信息和脚本命令
...
- **`src/`** — 主要的源代码文件夹，开发时大部分时间在这里工作。
  - **`components/`** — 可复用的 Vue 组件文件（`.vue` 单文件组件）。
  - **`views/`** — 页面级组件，通常作为路由的目标组件。
  - **`router/`** — Vue Router 路由配置（如果在脚手架中选择了路由功能）。
  - **`stores/`** — Pinia 状态管理 store（如果在脚手架中选择了 Pinia）。
  - **`assets/`** — 静态资源，如图片和 CSS 文件。
  - **`App.vue`** — 根组件，所有其他组件都在此组件内被引用。
  - **`main.js`** — 入口文件，创建 Vue 实例并挂载到 DOM。
- **`public/`** — 该目录下的文件会被原样提供，不经过 Vite 处理。
- **`index.html`** — 主 HTML 文件，包含 `<div id="app"></div>`，Vue 实例挂载于此。
- **`vite.config.js`** — Vite 配置文件，提供快速的开发服务器和热模块替换 (HMR)。

:::tip
这个结构为 Vue 项目提供了良好的组织和模块化的起点，使得开发和维护都更加方便。
:::
